...
#
# Uso:
#   core-ssh-msg.sh <kz|kora|samy|pau|cp> "texto"
...
dest="${1:-}"
shift || true
[[ -n "${dest}" ]] || { echo "uso: $0 <kz|kora|samy|pau|cp> \"texto\"" >&2; exit 2; }
...
lookup() {
  case "$1" in
    ...
    cp)        echo "lalo@192.168.1.100" "$HOME_CP_INBOX" ;;
    *) return 1 ;;
  esac
}
...
# Samy confirmó 17-ago: mismo path, host 305v4.
...
HOME_CP_INBOX="${HOME_CP_INBOX:-/home/lalo/kz/presence/cp-inbox}"
